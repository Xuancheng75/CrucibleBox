...
export function inspectPluginRendererRpcPayload(
  ...
  // 委托共享内核；fail 适配器保持 renderer 结构化 issue（{code,message,path}）语义
  return inspectRpcPayload(value, budget, (code, message, path) =>
    fail(code as PluginRendererRpcErrorCode, message, path)
  )
}
...
export class PluginRendererRpcPendingRequests {
  private readonly tracker: PendingRequestTracker
  ...
  constructor(readonly limit = PLUGIN_RENDERER_RPC_MAX_PENDING_REQUESTS) {
    ...
    this.tracker = new PendingRequestTracker(
      limit,
      {
        duplicateRequestId: () =>
          new PluginRendererRpcValidationError(
            'INVALID_REQUEST_ID',
      ...
      (requestId) => validatePluginRendererRpcRequestId(requestId)
    )
  }
  ...
  add(requestId: string): void {
    this.tracker.add(requestId)
  }

  delete(requestId: string): boolean {
    return this.tracker.delete(requestId)
  }

  has(requestId: string): boolean {
    return this.tracker.has(requestId)
  }
