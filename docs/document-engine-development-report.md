...
Document Engine 已完成可验收版本：插件已注册到宿主，前端通过 self-contained renderer 构建，所有文档处理请求均经 Rust trusted service，OCR 由独立 Rust + PaddleOCR ONNX worker 执行。当前实现不依赖 Python、pip、Conda、CUDA Toolkit 或 Paddle Python 环境。
...
- 插件：`plugins/document-engine/plugin.json`、`src/main.ts`、`src/engine-api.ts`、`src/renderer.tsx`、`src/renderer-entry.tsx`、`tests/skeleton.test.ts`、`dist/main.js`、`dist/renderer.js`。
...
| 模型管理 | 通过；list、local install、HTTPS allow-list + SHA-256 remote install/update、safe remove |
...
- `src-tauri`: fmt、clippy `-D warnings`、workspace tests **171 passed**（含 Document Engine renderer/backend RPC smoke）。
- `ocr-worker`: **7 passed**。
- `plugins/document-engine`: TypeScript typecheck、renderer build、Vitest **10 passed**。
- 宿主 Vitest：**274 passed**；供应链 Node tests：**16 passed**；ESLint 与三层宿主 TypeScript typecheck 通过。
- renderer self-contained 校验：`document-engine` **234,190 bytes**，无 CommonJS/import/eval，runtime mount 标记存在。
- trusted policy 校验：`document-engine 0.1.0` digest `0bb2c3e8ba04ba9b3358ce4a935877f7c1ba061d57861b26f4d761848bac7b1f`。
- Tauri 前端 Vite production build：**3055 modules transformed，成功**。
- Windows x64 NSIS：`src-tauri/target/release/bundle/nsis/CrucibleBox_1.9.17_x64-setup.exe` 生成成功；临时安装验证确认 OCR Worker 与 PDFium 均随包落地。
...
3. 模型不随安装包分发，首次使用需通过配置的模型目录或 Model Manager 安装；远程安装强制 HTTPS、域名白名单与 SHA-256。
