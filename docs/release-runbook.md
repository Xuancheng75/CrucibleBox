...
- `plugins.json` 必须携带插件展示元数据（名称、描述、发布者、图标）以及当前版本 tag 的固定 GitHub Release 下载 URL；插件市场从宿主对应的 `tauri-vX.Y.Z[-beta.N|-rc.N]` Release 获取目录，不使用 `tauri-stable`/`tauri-beta` 滚动地址；滚动清单仅供 updater 通道使用，不配置镜像地址。
