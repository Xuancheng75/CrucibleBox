...
> **实施状态**：2.0.0-beta.7 / Document Engine 0.8.0 已按本架构继续落地；已完成实现、门禁和限制以
...
- Document Engine 0.1.2 随插件分发约 15.6 MiB 的 CPU 默认 PP-OCRv4 模型包，首次激活时校验并复制到配置目录。
- 模型更新或内置资源不可用时，通过受信的 jsDelivr/GitHub Release/上游 HTTPS 源顺序回退下载。
- 每个文件固定 SHA-256，下载使用临时文件和原子提交；失败源不会降低校验或安全要求。
- 下载进度和错误来源由 Model Manager 展示；用户仍可从本地路径安装自定义模型。
