...
- 应用启动后会在当前主题内检查稳定版更新，并以统一风格弹窗询问用户立即更新或稍后提醒；检查失败不阻断启动。
- 插件市场下载任务继续在任务中心运行，保留断点续传、重试、临时文件恢复、代理策略和 SHA-256 校验；下载进度不改变市场卡片或侧栏布局。
- GIF 编辑器缩略图按尺寸生成，图层撤销/重做历史增加字节上限，避免大尺寸帧编辑时内存无界增长。
- Document Engine 0.10.0 保持原生文本优先和稳定 Hybrid Chunk 长度算法，继续加强结构化公式、章节树、XML/DOCX 合法性和扫描 PDF 回退链路。
