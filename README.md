...
- 主程序版本 1.7.3；数据库 schema v3（`plugins.sort_order`）；生产仅 better-sqlite3 单引擎（sql.js 已从生产移除，保留为测试/离线恢复工具）；UniEnv trusted service 已物理隔离至宿主侧 `plugin-system/trusted-services/unienv/`。
- 六插件版本：diary 0.4.11、dice-roller 0.1.6、gif-editor 0.3.8、theme-manager 0.1.12、turntable 0.1.10、unienv 0.5.7。
- 测试：宿主 36 个测试文件、261 项；六插件 199 项（theme-manager 新增 9 项最小测试）；供应链 16 项全部通过（当前 0 跳过）。
