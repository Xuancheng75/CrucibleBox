...
- **Tauri 发布链**（`tauri-release.yml`，`tauri-v*` tag）：NSIS 安装器（WebView2
  ...
  验证基线为 **v2.0.0-beta.6**，稳定/测试通道分别发布到 `tauri-stable` / `tauri-beta` 滚动元数据。
...
- 正式插件清单以 `scripts/plugin-catalog.json` 为准，目前包含 Document Engine（0.7.0，支持最多 2000 页 PDF、PDFium 原生文字优先、统一文本清洗、TOC 隔离、章节树、公式块、DOCX XML 校验、Markdown/TXT/JSON 导出、真实 PDF 物理拆分、JSONL 文本分块、可选择输出文件夹与内置 CPU OCR 默认模型）、
