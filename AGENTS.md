...
- **CrucibleBox**（npm 包名 `cruciblebox`，Rust crate `cruciblebox`）：**Tauri 2.11.x + Rust + React 18 + Ant Design 5 + zustand + rusqlite** 构建的 Windows 10/11 x64 可扩展工具箱（当前发布基线 **1.9.24**；Electron 43/React 19/Ant Design 6 为已冻结的历史运行线）。
...
1.8.X Tauri 迁移（骨架/DB/sidecar/renderer/发布链）→ 1.9.X 插件独立化 + 宿主收敛 → **1.9.2 冻结 Tauri 为唯一运行线并发布首个 Tauri 正式版**（Electron 分支归档）。当前发布基线：**1.9.24**。逐版本变更见 `docs/changelog.md`；UniEnv 在线版本源决策见 `docs/adr-0021-unienv-online-version-feeds.md`。
