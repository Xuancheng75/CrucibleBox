...
- 当前基线（1.7.2）：宿主 36 文件/261 项、六插件 199 项（theme-manager 新增 9 项最小测试）、供应链 16 项测试全绿；数据库 schema v3（`plugins.sort_order`）；生产仅 better-sqlite3 单引擎（sql.js 已从生产移除，保留为测试/离线恢复工具）；UniEnv trusted service 已物理隔离至宿主侧 `plugin-system/trusted-services/unienv/`（固定文件集/摘要/fail-closed 不变）；插件 SDK v2 已冻结（API 变更须走 v3 提案）；自动更新默认关闭，配置 GitHub Release 后启用。
